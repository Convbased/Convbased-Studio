...
from utils import load_audio, load_embedding
from preparing_files import generate_config, generate_filelist
    ...
    def process_files(self, files, f0_method, hop_length, device, threads):
        ...
        if f0_method == "rmvpe":
            self.model_rmvpe = RMVPE0Predictor(
                os.path.join("models", "predictors", "rmvpe.pt"),
                device=device,
            )
...
def run_embedding_extraction(
    ...
    devices_str = ", ".join(devices)
    print(
        f"Starting embedding extraction cores on {devices_str}..."
    )
    start_time = time.time()
    with concurrent.futures.ProcessPoolExecutor(max_workers=len(devices)) as executor:
        tasks = [
